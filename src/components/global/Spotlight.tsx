...
export interface SpotlightProps {
  ...
  actions: {
    ...
    openFinder?: () => void;
    openProjectsLab?: () => void;
    openFounderHQ?: () => void;
    openMuseum?: () => void;
    openResearch?: () => void;
    openKnowledge?: () => void;
    openBuildMode?: () => void;
    openCareerControl?: () => void;
...
export default function Spotlight({ isOpen, onClose, actions }: SpotlightProps) {
  ...
  const items = useMemo<SpotlightItem[]>(() => {
    ...
    const publicationItems: SpotlightItem[] = userConfig.publications.map((p, idx) => ({
      id: `publication:${idx}`,
      title: p.title,
      subtitle: `${p.venue} • ${p.year}`,
      category: 'Research',
      keywords: [p.venue, p.doi ?? '', ...(p.awards ?? [])],
      icon: <IoBookOutline className="text-gray-300" />,
      action: () => actions.openNotesSection('publications')
    }));

    const certItems: SpotlightItem[] = userConfig.certifications.map((c, idx) => ({
      id: `certification:${idx}`,
      title: c.title,
      subtitle: `${c.issuer}${c.category ? ` • ${c.category}` : ''}`,
      category: 'Certifications',
      keywords: [c.issuer, c.category ?? ''],
      icon: <IoDocumentTextOutline className="text-gray-300" />,
      action: () => actions.openNotesSection('certifications')
    }));
    ...
    const quickActions: SpotlightItem[] = [
      ...
      {
        id: 'action:founder-hq',
        title: 'Open Founder HQ',
        subtitle: 'Tokenistt startup headquarters',
        category: 'Apps',
        icon: <IoTerminalOutline className="text-gray-300" />,
        action: () => actions.openFounderHQ?.(),
      },
      {
        id: 'action:finder',
        title: 'Open Finder',
        subtitle: 'Browse projects, research, awards',
        category: 'Apps',
        icon: <IoBookOutline className="text-gray-300" />,
        action: () => actions.openFinder?.(),
      },
      {
        id: 'action:research',
        title: 'Research Center',
        subtitle: 'Publications and papers',
        category: 'Apps',
        icon: <IoBookOutline className="text-gray-300" />,
        action: () => actions.openResearch?.(),
      },
      {
        id: 'action:terminal',
        title: 'Open Terminal',
      ...
      {
        id: 'action:notes-publications',
        title: 'Open Notes: Research & Publications',
        subtitle: 'Jump to papers and research awards',
        category: 'Actions',
        icon: <IoBookOutline className="text-gray-300" />,
        action: () => actions.openNotesSection('publications'),
      },
      {
        id: 'action:notes-certifications',
        title: 'Open Notes: Certifications',
        subtitle: 'Jump to certifications',
        category: 'Actions',
        icon: <IoDocumentTextOutline className="text-gray-300" />,
        action: () => actions.openNotesSection('certifications'),
      },
      {
        id: 'action:notes',
        title: 'Open Notes',
        subtitle: 'Education, Experience, Skills, Research',
        category: 'Actions',
        icon: <IoBookOutline className="text-gray-300" />,
      ...
      {
        id: 'link:blog',
        title: 'Read my Blog (Medium)',
        subtitle: 'When Machines Learned to Remember',
        category: 'Links',
        icon: <FaRegFileAlt className="text-gray-300" />,
        action: () => window.open('https://medium.com/@tiwariaditya005/when-machines-learned-to-remember-5769d86c1b49', '_blank', 'noopener,noreferrer'),
      },
    ];
  ...
  const grouped = useMemo(() => {
    const order = ['Actions', 'Projects', 'Experience', 'Education', 'Skills', 'Research', 'Certifications', 'Links'];
