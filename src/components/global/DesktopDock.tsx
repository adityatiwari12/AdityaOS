...
import { BsGithub, BsSpotify, BsFilePdf, BsStickyFill, BsLinkedin, BsCalendar, BsPlayCircleFill, BsGraphUp } from 'react-icons/bs';
...
const DesktopDock = ({ onTerminalClick, onNotesClick, onGitHubClick, onContactClick, onIntroClick, onVideosClick, onContributionsClick, activeApps }: DesktopDockProps) => {
  ...
  const handleCalendarClick = () => {
    // Use Calendly if configured, otherwise fall back to LinkedIn for scheduling/contact
    const url = userConfig.contact.calendly || userConfig.social.linkedin;
  ...
  const icons = [
    { id: 'intro', label: 'Intro', onClick: onIntroClick, icon: RiTerminalBoxFill, color: 'from-gray-800 to-gray-600', active: activeApps.intro },
    { id: 'github', label: 'My Projects', onClick: onGitHubClick, icon: BsGithub, color: 'from-black to-black/60', active: activeApps.github },
    { id: 'videos', label: 'Project Videos', onClick: onVideosClick, icon: BsPlayCircleFill, color: 'from-rose-600 to-rose-400', active: activeApps.videos },
    { id: 'contributions', label: 'GitHub Activity', onClick: onContributionsClick, icon: BsGraphUp, color: 'from-cyan-600 to-cyan-400', active: activeApps.contributions },
    ...
    { id: 'terminal', label: 'AI Terminal', onClick: onTerminalClick, icon: RiTerminalFill, color: 'from-black to-black/60', active: activeApps.terminal },
  ];

  return (
    <>
      <nav aria-label="Dock" className="fixed bottom-0 left-0 right-0 hidden md:flex justify-center pb-4 z-100">
        <div ref={dockRef} className="bg-gray-600/50 backdrop-blur-sm rounded-2xl px-4 py-3 shadow-xl">
